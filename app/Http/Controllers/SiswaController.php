            ...
            return Datatables::of($data)//->make(true);
                 ->addColumn('action', function($data) { // menambahkan tabel berisikan 2 fungsi. Penambahan kolom jenis ini dapat dilakukan
                    return view('datatable._action', [   //  apabila informasi dari kolom berada diluar dari tabel  yang ingin di tampilkan 
                        ...
                        'confirm_message'    => 'Yakin Mau Menghapus Siswa' . $data->nama_lengkap . '?' //menampilkan pop-up sebelum data akan dihapus
                    ]);
				})
    ...
    public function destroy(Request $request, $id)
    {
		...
		if($dataTabungan == null){
			...
			Session::flash("flash_notification", [
				"level" => "success",
				"icon" => "fa fa-check",
				"message" => "Siswa ".$nama_siswa." Berhasil Dihapus!"
			]);
		}
			...
			Session::flash("flash_notification", [
				"level" => "warning",
				"icon" => "fa fa-check",
				"message" => "Gagal! Siswa ".$nama_siswa." Digunakan dalam Tabel Tabungan!",
			]);
		}
