...
class LaporanController extends Controller
{
    public function index(Request $request, Builder $htmlBuilder)
    {
		...
		$tabsis = $request->get('jenistab'); 
		$format = '%s';
		$tabsis2 = sprintf($format, $tabsis);
		//dd($tabsis);
		//dd($tabsis2);
		//$data = DB::table('transaksi')->whereBetween(DB::raw('DATE(created_at)'), array($from_date, $to_date))->get();
		//$data = DB::table('transaksi')->where('jenis_tabungan','wisata')->get();
		//$data = DB::table('transaksi')->where('jenis_tabungan',$tabsis2)->get();
		$data = DB::table('transaksi')->where('jenis_tabungan',$tabsis2)->get();
		//dd($data);
        if ($request->ajax()) {
		//if (true) {
